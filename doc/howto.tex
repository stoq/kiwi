...
\author{Christian Reis\\Async Open Source, Brazil}
\authoraddress{kiko@async.com.br\\http://www.async.com.br/projects/kiwi/}
\date{November, 2002}
...
All you need to use Kiwi is Python and PyGTK. It's free software
licensed under the LGPL and can be used and embedded in any application.
\end{abstract}
...
\small{(This example application, \file{examples/Diary/diary3.py}, has
99 lines, not counting the Glade XML specification)}
\end{center}
...
James Henstridge's
\citetitle[http://www.daa.com.au/\%7ejames/pygtk/]{PyGTK}, an excellent
...
libglade support, which allows you to graphically specify (using Glade)
...
though it is complex in parts (even crufty in a few spots), I consider
...
in Stoq, and over the past months has grown to become very useful (or so
the users say :-). I welcome feedback and have been very fast to respond
to requests and bug reports, so you can be sure your application can
rely on a library that is actively supported and that its author has a
...
\begin{itemize}
  \item  \citetitle[http://www.python.org/]{Python} 1.5 or newer
  \item  \citetitle[http://www.gtk.org/]{GTK+} 1.2\footnote{GTK+ and
  PyGTK 2.0 support is not implemented yet. I've been trying to
  stabilize the Kiwi API for a while now, and when the change to GTK+
  2.0 does happen, I will make great effort to keep things backwards
  compatible.}
  \item  \citetitle[http://www.daa.com.au/\%7ejames/gnome/]{libglade}
  0.12 (or higher)
  \item  \citetitle[http://www.daa.com.au/\%7ejames/pygtk/]{PyGTK} 0.6
\end{itemize}
